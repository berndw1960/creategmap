...
def pre_comp():
  ...
  if config.get('navmap', 'pre_comp') == "yes":
    if config.get('navmap', 'sea') == "latest":
      ...
      if ExitCode == False:
        url = "http://osm2.pleiades.uni-wuppertal.de/sea/latest/" + (sea_rev) + ".zip"
    ...
    if config.get('navmap', 'bounds') == "latest":
      target = http.client.HTTPConnection("osm2.pleiades.uni-wuppertal.de")
      target.request("GET", "/bounds/latest/")
      htmlcontent =  target.getresponse()
      data = htmlcontent.read()
      data = data.decode('utf8')
      ...
      if ExitCode == False:
        url = "http://osm2.pleiades.uni-wuppertal.de/bounds/latest/" + (bounds_rev) + ".zip"
